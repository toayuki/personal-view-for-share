...
def _get_viewable_category_ids(user_id) -> list[str] | None:
    key = str(user_id)
    ...
    try:
        ...
        if res.status_code == 200:
            ids = res.json().get("category_ids", [])
            _viewable_cache[key] = (ids, datetime.now() + _CACHE_TTL)
            return ids
    except Exception:
        pass
    return None
...
def _invalidate_viewable_cache(user_id) -> None:
...
def _get_user_categories(user_id, role: str) -> list:
    """ユーザー権限に応じたカテゴリ一覧を返す"""
...
app.middleware("http")(audit_logger.make_audit_middleware(_decode_token, API_URL))
