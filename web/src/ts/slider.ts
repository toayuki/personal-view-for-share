 ...
 */
function playSlideVideo(slide: HTMLElement): void {
  const video = slide.querySelector<HTMLVideoElement>('.slide-video');
  if (!video) return;
 ...
 */
function syncSlideVideos(sw: JQuery): void {
  sw.find('.slide').each(function () {
    const video = this.querySelector<HTMLVideoElement>('.slide-video');
    if (!video) return;
...
export function slideshowSwitch(
  ...
  if (!sw.data('cachedSlides')) sw.data('cachedSlides', sw.find('.slide'));
  const slides: JQuery = sw.data('cachedSlides');
  ...
  const timeout: number = sw.data('timeout');
  clearTimeout(timeout);

  sw.data('wait', true);

  if (!sw.data('cachedPages')) sw.data('cachedPages', sw.find('.pagination .item'));
  const pages: JQuery = sw.data('cachedPages');
  ...
  let newSlideRight: number | string;
  let newSlideLeft: number | string;
  let newSlideImageRight: number | string;
  let newSlideImageLeft: number | string;
  let newSlideImageToRight: number | string;
  let newSlideImageToLeft: number | string;
  let newSlideContentLeft: number | string;
  let newSlideContentRight: number | string;
 ...
 */
function slideshowNext(sw: JQuery, previous: boolean, auto: boolean): void {
 ...
 */
function runIntroSequence(sw: JQuery, onComplete: () => void): void {
  const slides = sw.find('.slide');
  ...
  Promise.all(loadPromises).then(() => {
    ...
    categorySlides.each(function (i) {
      ...
      playSlideVideo(this as HTMLElement);
    });
...
$(document).ready(() => {
  ...
  slideshow.on('click', '.slide .btn', function (e: JQuery.ClickEvent) {
    e.preventDefault();
