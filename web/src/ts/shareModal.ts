import { openNotificationModal } from './modal.js';
...
function initShareModal(): void {
      ...
      <div class="modal">
        ...
        <div class="share-square-btns">
          ...
          <button class="share-square-btn" id="share-link-btn">
            <i class="fa-solid fa-link"></i><span>リンク</span>
          </button>
        </div>
  ...
  shareModal.querySelector('#share-qr-btn')!.addEventListener('click', () => {
    ...
    const container = shareModal!.querySelector<HTMLElement>('#share-qr-container')!;
...
async function openShareModal(categoryId: string): Promise<void> {
  ...
  const status = shareModal!.querySelector<HTMLElement>('#share-modal-status')!;
...
document.addEventListener('click', (e) => {
  const btn = (e.target as HTMLElement).closest<HTMLElement>('.category-share-btn');
  if (!btn) return;
  e.stopPropagation();
