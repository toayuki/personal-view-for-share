...
const categoryId = result === null || result === void 0 ? void 0 : result.dataset.categoryId;
            ...
            fetchConversionStatuses(videoItems.map(el => el.dataset.storedFileName))
                ...
                videoItems.forEach(el => {
                    ...
                    if (d && d.status !== 'done' && d.status !== 'error') {
                        ...
                        _updateBar(el, (_a = d.progress) !== null && _a !== void 0 ? _a : 0, d.status);
                    }
                });
...
export function createGridItem(item) {
    ...
    editLink.dataset.title = (_a = item.title) !== null && _a !== void 0 ? _a : '';
    ...
    img.src = `/personal-web/contents/${categoryId}/thumbnail/${(_b = item.thumbnail_file_name) !== null && _b !== void 0 ? _b : item.file_name}`;
    a.appendChild(img);
...
export function addConvertingOverlay(item) {
    if (item.classList.contains('converting'))
        return;
...
function _updateBar(item, progress, status) {
    ...
    if (bar)
        bar.style.width = isWaiting ? '0%' : `${progress}%`;
    if (label)
        label.textContent = isWaiting ? '待機中' : `変換中 ${progress}%`;
    if (label)
    ...
    const interval = setInterval(async () => {
        ...
        try {
            ...
            converting.forEach(item => {
                ...
                if (!d)
                    return;
                _updateBar(item, (_a = d.progress) !== null && _a !== void 0 ? _a : 0, d.status);
                if (d.status === 'done' || d.status === 'error') {
                    item.classList.remove('converting');
                    (_b = item.querySelector('.converting-overlay')) === null || _b === void 0 ? void 0 : _b.remove();
                }
            });
        }
        catch (_b) { }
